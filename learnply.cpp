...
void print_geometry_summary(Polyhedron* poly, const std::string& model_name);


struct Corner {
    Triangle* tri;
    int corner_index; // 0, 1, or 2
};

std::vector<Corner> build_corner_list(Polyhedron* poly) {
    std::vector<Corner> corner_list;
    for (int i = 0; i < poly->ntris; ++i) {
        for (int j = 0; j < 3; ++j) {
            corner_list.push_back({poly->tlist[i], j});
        }
    }
    return corner_list;
}
...
void compute_angle_deficit(Polyhedron *poly) {
    for (int i = 0; i < poly->nverts; i++) {
        ...
        for (int j = 0; j < v->ntris; j++) {
            Triangle *t = v->tris[j];
            int vidx = poly->face_to_vertex_ref(t, v);
            Vertex *v1 = t->verts[(vidx + 1) % 3];
            Vertex *v2 = t->verts[(vidx + 2) % 3];

            icVector3 a(v1->x - v->x, v1->y - v->y, v1->z - v->z);
            icVector3 b(v2->x - v->x, v2->y - v->y, v2->z - v->z);

            double len_a = sqrt(dot(a, a));
            double len_b = sqrt(dot(b, b));
        ...
        v->angle_deficit = (2.0 * M_PI) - angle_sum;
		std::cout << "Vertex " << i << " angle deficit: " << v->angle_deficit << std::endl;

    }
}

		int compute_euler_characteristic(Polyhedron* poly) {
			int V = poly->nverts;
			int E = poly->nedges;
			int F = poly->ntris;
			return V - E + F;
		...
		double compute_total_angle_deficit(Polyhedron* poly) {
			double total_deficit = 0.0;
			for (int i = 0; i < poly->nverts; ++i) {
				total_deficit += poly->vlist[i]->angle_deficit;
			}
			return total_deficit;
		}

		void compute_discrete_gaussian_curvature(Polyhedron* poly) {
			for (int i = 0; i < poly->nverts; ++i) {
				Vertex* v = poly->vlist[i];
				double area_sum = 0.0;
				...
				if (area_sum > 1e-6)
					v->gaussian_curvature = v->angle_deficit / area_sum;
				else
					v->gaussian_curvature = 0.0;
			}
		}
		...
		int compute_num_handles(Polyhedron* poly) {
			int chi = compute_euler_characteristic(poly);
			return (2 - chi) / 2;
		}

		void print_geometry_summary(Polyhedron* poly, const std::string& model_name) {
			std::cout << "====== Geometry Summary: " << model_name << " ======" << std::endl;
		
			int V = poly->nverts;
			int E = poly->nedges;
			int F = poly->ntris;
			int chi = compute_euler_characteristic(poly);
			double total_deficit = compute_total_angle_deficit(poly);
			compute_discrete_gaussian_curvature(poly); // Optional for side usage
			int handles = compute_num_handles(poly);
		
			std::cout << "Vertices (V):             " << V << std::endl;
			std::cout << "Edges (E):                " << E << std::endl;
			std::cout << "Faces (F):                " << F << std::endl;
			std::cout << "Euler Characteristic (χ): " << chi << std::endl;
			std::cout << "Total Angle Deficit:      " << total_deficit << std::endl;
			std::cout << "Number of Handles (g):    " << handles << std::endl;

			std::cout << "==============================================" << std::endl;

			std::cout << "---------------------------------------------" << std::endl;
			std::cout << "Vertex ID\tGaussian Curvature\tAngle Deficit" << std::endl;
			std::cout << "---------------------------------------------" << std::endl;

			for (int i = 0; i < std::min(poly->nverts, 20); ++i) {
				std::cout << std::scientific << std::setprecision(10)
						  << i << "\t\t"
						  << poly->vlist[i]->gaussian_curvature << "\t"
						  << poly->vlist[i]->angle_deficit << std::endl;
			}
...

{
    ...
    for (i = 0; i < this_poly->ntris; i++) {
		...
		switch (display_mode) {
		case 0: {  // Polygon ID Coloring
			double percentage = (double)i / (double)(this_poly->ntris - 1);
			double col[3];
			color_mapping(percentage, col);  // Use the already-defined function
		...
		case 1: {  // Vertex-based RGB coloring
			glDisable(GL_LIGHTING);  // <- Add this line
			glBegin(GL_POLYGON);
			for (j = 0; j < 3; j++) {
				Vertex *temp_v = temp_t->verts[j];
		
				float r = (temp_v->x - this_poly->xmin) / (this_poly->xmax - this_poly->xmin);
				float g = (temp_v->y - this_poly->ymin) / (this_poly->ymax - this_poly->ymin);
				float b = (temp_v->z - this_poly->zmin) / (this_poly->zmax - this_poly->zmin);
			...
			glEnd();
			glEnable(GL_LIGHTING);  // <- Re-enable afterward
			break;
		}
	
		case 2: {
			float r = fabs(temp_t->normal.entry[0]);
			float g = fabs(temp_t->normal.entry[1]);
			float b = fabs(temp_t->normal.entry[2]);
			glBegin(GL_POLYGON);
			glColor3f(r, g, b);
			for (j = 0; j < 3; j++) {
				Vertex* temp_v = temp_t->verts[j];
		...
		case 3: {  // 3D checkerboard coloring
			glDisable(GL_LIGHTING);  // <- Add this
			...
			for (j = 0; j < 3; j++) {
				...
				int xi = (int)floor(temp_v->x / L);
				int yi = (int)floor(temp_v->y / L);
				int zi = (int)floor(temp_v->z / L);
		
				float r = (xi % 2 == 0) ? 1.0f : 0.0f;
				float g = (yi % 2 == 0) ? 1.0f : 0.0f;
				float b = (zi % 2 == 0) ? 1.0f : 0.0f;
			...
			glEnd();
			glEnable(GL_LIGHTING);  // <- Re-enable here
			break;
		}

		case 4: {  // Visualize angle deficit
			glDisable(GL_LIGHTING);  // <<-- Add this
			glBegin(GL_POLYGON);
			for (j = 0; j < 3; j++) {
				Vertex *temp_v = temp_t->verts[j];
				double deficit = temp_v->angle_deficit;
		
				// Normalize for coloring
				float color = fmin(fmax(deficit / (2 * M_PI), 0.0), 1.0);
				glColor3f(color, 0.0, 1.0 - color);  // Red = high, Blue = low
		...
		case 5: {  // Gaussian curvature sign visualization (Red = positive, Blue = negative)
			glDisable(GL_LIGHTING);  // <<-- Add this
			glBegin(GL_POLYGON);
			for (j = 0; j < 3; j++) {
				Vertex *temp_v = temp_t->verts[j];
				double curvature = temp_v->angle_deficit;
		
				if (curvature > 0) glColor3f(1.0, 0.0, 0.0);  // red
		...
		case 7: {  // Normal-based coloring per vertex
			glDisable(GL_LIGHTING);
			glBegin(GL_POLYGON);
			for (j = 0; j < 3; j++) {
				Vertex* temp_v = temp_t->verts[j];
				float r = fabs(temp_v->normal.entry[0]);
				float g = fabs(temp_v->normal.entry[1]);
				float b = fabs(temp_v->normal.entry[2]);
				glColor3f(r, g, b);
				glNormal3d(temp_v->normal.entry[0], temp_v->normal.entry[1], temp_v->normal.entry[2]);
				glVertex3d(temp_v->x, temp_v->y, temp_v->z);
			}
...
int main(int argc, char *argv[])
{
  ...
  this_file = fopen("icosahedron.ply", "r");
	...
	compute_angle_deficit(poly);
	print_geometry_summary(poly, "icosahedron");


	poly->calc_bounding_sphere();
	poly->calc_face_normals_and_area();
	poly->average_normals();
	compute_angle_deficit(poly);  // Compute angle deficits for curvature visualization
...
for (int i = 1; i < nverts; i++) {
    if (vlist[i]->x < xmin) xmin = vlist[i]->x;
    if (vlist[i]->x > xmax) xmax = vlist[i]->x;
    if (vlist[i]->y < ymin) ymin = vlist[i]->y;
    if (vlist[i]->y > ymax) ymax = vlist[i]->y;
    if (vlist[i]->z < zmin) zmin = vlist[i]->z;
...
void Polyhedron::calc_bounding_sphere()
{
  ...
  for (i=0; i<nverts; i++) {
    if (i==0)  {
      ...
      if (vlist[i]->x < min.entry[0]) min.entry[0] = vlist[i]->x;
      if (vlist[i]->x > max.entry[0]) max.entry[0] = vlist[i]->x;
      if (vlist[i]->y < min.entry[1]) min.entry[1] = vlist[i]->y;
      if (vlist[i]->y > max.entry[1]) max.entry[1] = vlist[i]->y;
      if (vlist[i]->z < min.entry[2]) min.entry[2] = vlist[i]->z;
      if (vlist[i]->z > max.entry[2]) max.entry[2] = vlist[i]->z;
    }
  }

  // Assign bounding box to class variables
  xmin = min.entry[0];
  xmax = max.entry[0];
  ymin = min.entry[1];
  ymax = max.entry[1];
    ...
    case 27:
      ...
      break;
		...
		case '0':
			...
			case '1':
			display_mode = 1; // Vertex-based RGB coloring
			display();
			break;
	
		case '2':
			display_mode = 2; // Normal-based coloring
			display();
			break;
		case '3':
            display_mode = 3;
